...
def main():
    ...
    # save information for monitoring
    runs_arr = array('f')
    scale_factors = array('f')

    outscale_file = open("cdc_ascales.txt","w")
    ...
    for run in runs:
        ...
        try:
            print f.GetName()
            gROOT.ProcessLine(".x CDC_gains.C(1)")
            os.system("cat cdc_new_ascale.txt")
        ...
        a_scales = []
        with open("cdc_new_ascale.txt") as f:
            for line in f:
                a_scales = line.strip().split()
                break

        runs_arr.append( float(run) )
        scale_factors.append( float(a_scales[0]) )

        outscale_file.write("%d %s\n"%(run,a_scales[0]))
        continue
