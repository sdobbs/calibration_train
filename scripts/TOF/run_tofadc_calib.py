...
def main():
    ...
    for run in runs:
        ...
        #fname = "/work/halld/data_monitoring/RunPeriod-2017-01/mon_ver12/rootfiles/hd_root_%06d.root"%run
        fname = "/home/gxproj3/volatile/2017-01/ver12/hd_root_%06d.root"%run

        try:
            print fname
            gROOT.ProcessLine(".x $HALLD_HOME/src/plugins/Calibration/HLDetectorTiming/FitScripts/ExtractTDCADCTiming.C(\"%s\",%d,\"%s\")"%(fname,run,"default"))
            os.system("ccdb add /TOF/adc_timing_offsets -v %s -r %d-%d tof_adc_timing_offsets.txt"%("default",run,run))
