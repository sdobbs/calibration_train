...
def main():
    ...
    for run in runs:
        ...
        for plane in xrange(1,hfdctdc.GetNbinsX()/2+1):
            ...
            plane_shifts.append( (mean1,mean2) )

        # okay, let's find the average shift - assume the first few planes dont' shift    
        avg = (plane_shifts[0][0] + plane_shifts[0][1] + plane_shifts[1][0] + plane_shifts[1][1]) / 4.
        n = 4.
        for plane in xrange(2,len(plane_shifts)):
            # don't keep 32 ns shifted slots
            if( abs(plane_shifts[plane][0]-avg) < 10. ):
                avg += (plane_shifts[plane][0]-avg)/n
                n+=1.
            if( abs(plane_shifts[plane][1]-avg) < 10. ):
                avg += (plane_shifts[plane][1]-avg)/n
                n+=1.
        
        # now let's fix the slots which are shifted more than the average
        print "AVERAGE = "+str(avg)
        for plane in xrange(2,len(plane_shifts)):
            if( abs(plane_shifts[plane][0]-avg) > 10. ):
                plane_shifts[plane] = (avg, plane_shifts[plane][1])
                (package, chamber) = FindFDCPackageChamber(plane+1)
                package_times_shifted[package-1] = True
                print "shift p = %d  c = %d"%(package, chamber)
                print " of %s"%plane_shifts[plane][0]
                for wire in xrange(48):
                    tdc_offsets[package-1][chamber-1][wire] = str(float(tdc_offsets[package-1][chamber-1][wire]) - float(plane_shifts[plane][0]))
            if( abs(plane_shifts[plane][1]-avg) > 10. ):
                plane_shifts[plane] = (plane_shifts[plane][0],avg)
                (package, chamber) = FindFDCPackageChamber(plane+1)
                package_times_shifted[package-1] = True
                print "shift p = %d  c = %d"%(package, chamber)
                print " of %s"%plane_shifts[plane][1]
                for wire in xrange(48):
                    tdc_offsets[package-1][chamber-1][wire+48] = str(float(tdc_offsets[package-1][chamber-1][wire+48]) - float(plane_shifts[plane][1]))


        # set offsets
        #for plane in xrange(len(plane_shifts)):
        #    (package, chamber) = FindFDCPackageChamber(plane+1)
        #    for wire in xrange(48):
        #        tdc_offsets[package-1][chamber-1][wire] += plane_shifts[plane][0];
        #    for wire in xrange(48):
        #        tdc_offsets[package-1][chamber-1][wire+48] += plane_shifts[plane][1];
