...
def main():
    ...
    for run in runs:
        print "===%d==="%run
        adc_toff_assignment = ccdb_conn.get_assignment("/PHOTON_BEAM/hodoscope/fadc_time_offsets", run, VARIATION)
        ...
        f = TFile("/cache/halld/RunPeriod-2017-01/calib/ver07/hists/Run%06d/hd_calib_verify_Run%06d_001.root"%(run,run))
        ...
        for i in xrange(1,htagm.GetNbinsX()+1):
            ...
            if math.fabs(tdiff) < 1. or math.fabs(tdiff) > 18.:
            #if math.fabs(tdiff) < 3. or math.fabs(tdiff) > 10.:
                continue
        ...
        # TEMP
        if 241 not in run_chan_errors:
            continue

        tmp = {}
        tmp[241] = run_chan_errors[241]
        run_chan_errors = tmp
