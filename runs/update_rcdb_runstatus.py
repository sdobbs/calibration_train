...
def main():
    ...
    parser.add_option("--approve", dest="status_approved", action="store_true",
                      help="Set status to 'approved' (1)")
    parser.add_option("--approve_long", dest="status_approved_long", action="store_true",
                      help="Set status to 'approved_long' (2)")
    parser.add_option("--calib", dest="status_calib", action="store_true",
                      help="Set status to 'calibration' (3)")
    ...
    # set statuses
    if options.status_approved:
        status = 1
    if options.status_approved_long:
        status = 2
    if options.status_calib:
        status = 3
